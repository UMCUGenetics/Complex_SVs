...
# Fig 4: HOXD example
# Fig S3: MP3738 SOX3 insertion
# Fig S5 A,B,C,D: Workflow 3D genome datasets > FOXG1
# Fig S7: SOX9 example
# Fig S8: FOXG1 example
...
output_folder <- "~/hpc/cog_bioinf/cuppen/project_data/Complex_svs/Results/Figures_raw/"
...
ini_file <- "~/hpc/cog_bioinf/cuppen/project_data/Complex_svs/Scripts/Integration/Inputs/Genes_SVs.ini"
...
Genes_SVs <- read.delim("~/hpc/cog_bioinf/cuppen/project_data/Complex_svs/Results/Integration/MP/Genes_SVs.txt")
# supplental figure S3A


## Fig 4: HOXD
conf <- read.delim("~/hpc/cog_bioinf/cuppen/project_data/Complex_svs/Scripts/Visualization/Conf_files/HOXD.conf", stringsAsFactors = F)
pdf(file = "~/hpc/cog_bioinf/cuppen/project_data/Complex_svs/Results/Figures_raw/4B_HOXD.pdf", width = 4, height = 3, pointsize = 8, family = "ArialMT")
...
HOXD_enhancers <- read.delim("~/hpc/cog_bioinf/cuppen/project_data/Complex_svs/Data/Enhancers/Raw/Limb/HOXD_Enhancers_mouse_hg19.bed", header = F, stringsAsFactors = F)
...
#text(x = (HOXD_enhancers[,2]+HOXD_enhancers[,3])/2, y = 4, labels = as.character(HOXD_enhancers[,4]), cex = 0.8, col = "#4B0082")


# breakpoint_plot(conf = conf, 
#                 patient = "UTR22", 
#                 chromosome = 2,
#                 xlim_plot = c(176.9e6,177.1e6), 
#                 Genes_SVs = "")
# 
# breakpoint_plot(conf = conf, 
#                 patient = "UTR22", 
#                 chromosome = 2,
#                 xlim_plot = c(176.8e6,177.2e6), 
#                 Genes_SVs = "")
dev.off()
...
# Superenhancers are obtained from Wilderman et al, Cell Reports (2018) High-Resolution Epigenomic Atlas of Human Embryonic Craniofacial Development
palate_superenhancers <- read.delim(paste(input_folder, "Data/Superenhancers/Superenhancers_Palate.bed", sep = ""), stringsAsFactors = F, header = F)
palate_superenhancers_insertion <- palate_superenhancers[which(palate_superenhancers$V1 == "chr9" & palate_superenhancers$V2 > 16489097 & palate_superenhancers$V3 < 16659203),]
palate_superenhancers_insertion$der_start <- palate_superenhancers_insertion$V2 - 16489097 + 139502953
palate_superenhancers_insertion$der_end <- palate_superenhancers_insertion$V3 - 16489097 + 139502953

pdf(file = "~/hpc/cog_bioinf/cuppen/project_data/Complex_svs/Manuscript/Fig_S3A_bottom.pdf", width = 5, height = 2.5, pointsize = 10)
...
rect(xleft = palate_superenhancers_insertion$der_start, xright = palate_superenhancers_insertion$der_end, 
     ybottom = 5.55, ytop = 5.95, border = "red", col = rgb(255,0,0, 100, maxColorValue = 255), lty = 1, lwd = 0.8)
...
pdf(file = "~/hpc/cog_bioinf/cuppen/project_data/Complex_svs/Manuscript/Fig_S3A_top.pdf", width = 5, height = 2.5, pointsize = 10)
breakpoint_plot(conf = SOX3_Conf, 
                patient = "MP3738", 
                chromosome = 9,
...
rect(xleft = palate_superenhancers$V2[palate_superenhancers$V1 == "chr9" ], xright = palate_superenhancers$V3[palate_superenhancers$V1 == "chr9" ], 
     ybottom = 5.55, ytop = 5.95, border = "red", col = rgb(255,0,0, 100, maxColorValue = 255), lty = 1, lwd = 0.8)
dev.off()
...
plot_phenomatch_small <- function(Patient,
  ...
  phenomatch_overview_2Mb[,2:ncol(phenomatch_overview_2Mb)] <- round(  phenomatch_overview_2Mb[,2:ncol(phenomatch_overview_2Mb)], 0)
  ...
  data_table <- ggplot(phenomatch_overview_melted, aes(x = Phenotype, y = Gene, fill = Score, label = phenoMatchScore)) + 
    ...
    scale_fill_gradient(low ="white", high = "#E41A1C", limits=c(0,30), na.value = "gray") +
...
pdf(file = "~/hpc/cog_bioinf/cuppen/project_data/Complex_svs/Manuscript/Fig_S4A.pdf", width = 4, height = 3, pointsize = 10)
...
# Fig S3B
conf <- read.delim("~/hpc/cog_bioinf/cuppen/project_data/Complex_svs/Visualization/conf/TADs_only.conf", stringsAsFactors = F)

pdf(file = "~/hpc/cog_bioinf/cuppen/project_data/Complex_svs/Manuscript/Fig_S3B.pdf", width = 3.5, height = 2.5, pointsize = 10)
breakpoint_plot(conf = conf, 
                patient = "35293", 
                chromosome = 14,
...
## Fig S3C
RNA_expression_data <- read.delim("~/hpc/cog_bioinf/cuppen/project_data/Complex_svs/Common_data/RNA/Schmitt/primary_cohort.gencode.hg19.FPKM.matrix")
...
ggsave(filename = "~/hpc/cog_bioinf/cuppen/project_data/Complex_svs/Manuscript/Fig_S3C_FOXG1_Expression.pdf", height= 50, width= 50, units = "mm")


## Fig S3D: Enhancers
conf <- read.delim("~/hpc/cog_bioinf/cuppen/project_data/Complex_svs/Visualization/conf/TADs_Enhancers.conf", stringsAsFactors = F)
pdf(file = "~/hpc/cog_bioinf/cuppen/project_data/Complex_svs/Manuscript/Fig_S3D.pdf", width = 3.5, height = 2, pointsize = 9)
...
## Fig S3E: Virtual 4C
conf <- read.delim("~/hpc/cog_bioinf/cuppen/project_data/Complex_svs/Visualization/conf/FOXG1_V4C.conf", stringsAsFactors = F)
pdf(file = "~/hpc/cog_bioinf/cuppen/project_data/Complex_svs/Manuscript/Fig_S3E.pdf", width = 3.5, height = 2, pointsize = 9)
...
## Fig S3F: PCHiC + DHS
conf <- read.delim("~/hpc/cog_bioinf/cuppen/project_data/Complex_svs/Visualization/conf/PCHiC.conf", stringsAsFactors = F)
pdf(file = "~/hpc/cog_bioinf/cuppen/project_data/Complex_svs/Manuscript/Fig_S3F.pdf", width = 3.5, height = 2, pointsize = 9)
...
conf <- read.delim("~/hpc/cog_bioinf/cuppen/project_data/Complex_svs/Visualization/conf/SOX9.conf", stringsAsFactors = F)

pdf(file = "~/hpc/cog_bioinf/cuppen/project_data/Complex_svs/Manuscript/Figures_raw/S6A_MP3760_SOX9_karyotype.pdf", width = 6, height = 3.5, pointsize = 8)
plot_der_karyotype(Patient = "MP3760", breakpoints = SVs, xlim = c(0,150e6))
dev.off()

pdf(file = "~/hpc/cog_bioinf/cuppen/project_data/Complex_svs/Manuscript/Figures_raw/S6B_MP3760_SOX9_regulation.pdf", width = 6, height = 4, pointsize = 9)
...
# 
# ## Deletions (not included for now)
...
## Figure 3C
pdf(file = paste(output_folder, "3C_Phenomatches_16085.pdf"), width = 3.5, height = 2.5, pointsize = 6)
plot_phenomatch_small(Genes_SVs = Genes_SVs[Genes_SVs$Classification %in% c("T1", "T2"),], Patient = "16085", phenomatch_folder = paste(input_folder, "Results/Integration/MP/Phenomatch/", sep = ""))
dev.off()
