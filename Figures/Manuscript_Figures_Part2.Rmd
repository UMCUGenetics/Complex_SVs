...
Junctions <- read.delim(paste(Project_folder, "/Data/SV/Denovo_junctions.txt", sep = ""), header = T, stringsAsFactors = F)

Junctions <- read.delim(paste(Project_folder, "/Data/SV/Table_S2.txt", sep = ""), header = T, stringsAsFactors = F, fill = T) #May fail
Patient_IDs <- Junctions[, c("ID", "Reference.ID" )]
...
Genes_SVs3$Driver2 <- ifelse(Genes_SVs3$Driver %in% c("T1", "T2"), "T1/T2", as.character(Genes_SVs3$Driver))
Genes_SVs3$Driver2 <- factor(Genes_SVs3$Driver2, levels = c("No","T3", "T1/T2"))
...
Distance_plot
if(Export_figures == TRUE){
  ggsave(filename = paste(Output_folder, "S5B_Distance_to_bp.pdf", sep = ""), height= 60, width= 55, units = "mm", device = cairo_pdf)
}
+ 
